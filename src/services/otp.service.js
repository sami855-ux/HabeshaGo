...
export const sendOTP = async (user, otpType = "login") => {
  ...
  // 3️⃣ Save OTP to DB (NO type stored)
  ...
  // 4️⃣ OTP message & subject based on type (EMAIL ONLY)
  const otpMessageMap = {
    login: "to log in to your account",
    verification: "to verify your account",
    resend: "to complete your previous verification",
  }
  ...
  const otpPurpose = otpMessageMap[otpType] || "to complete your request"

  const subject = subjectMap[otpType] || "Your OTP Code"
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; max-width: 480px; margin: auto; padding: 40px 30px; border-radius: 16px; background: linear-gradient(135deg, #ffffff 0%, #f8fafc 100%);">
  ...
  <p style="color: #4A5568; font-size: 16px; line-height: 1.6; margin-bottom: 24px;">
    Please use the following verification code ${otpPurpose}:
  </p>
  ...
  <div style="background: #FFFFFF; border-radius: 12px; padding: 32px 24px; margin: 32px 0; border: 1px solid #151516ff; text-align: center; box-shadow: 0 4px 12px rgba(0, 121, 107, 0.05);">
    <div style="color: #718096; font-size: 14px; font-weight: 600; letter-spacing: 0.5px; margin-bottom: 12px; text-transform: uppercase;">
      Verification Code
    </div>
    <div style="font-size: 40px; font-weight: 700; letter-spacing: 6px; color: #00796B; font-family: 'Courier New', monospace; padding: 12px; background: #f7fafc; border-radius: 8px; display: inline-block; min-width: 240px;">
      ${code}
    </div>
  </div>
